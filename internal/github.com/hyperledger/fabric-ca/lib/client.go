...
import (
	...
	"github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric-ca/api"
	"github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric-ca/lib/client/credential"
	x509cred "github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric-ca/lib/client/credential/x509"
	"github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric-ca/lib/common"
	"github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric-ca/lib/streamer"
	"github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric-ca/lib/tls"
...
func (c *Client) initHTTPClient(serverName string) error {
	...
	gmtr := new(gm_http.Transport)
	isGM := gm_x509.GetX509() == gm_x509.GetX509SM2() //TODO  here add gm flag
	if c.Config.TLS.Enabled {
		if isGM{
			log.Info("TLS Enabled")

			tlsConfig, err2 := tls.GetGMClientTLSConfig(&c.Config.TLS, c.csp)
			if err2 != nil {
			...
			gmtr.TLSClientConfig = tlsConfig
			c.gmhttpClient = &gm_http.Client{Transport: gmtr}
...
func (c *Client) SendReq(req *http.Request, result interface{}) (err error) {
    ...
    isGM := gm_x509.GetX509SM2() == gm_x509.GetX509()  //TODO add....
	var resp *http.Response
	if isGM && (c.gmhttpClient!= nil) {
		gmreq :=  (*gm_http.Request)(unsafe.Pointer(req))
		var gmresp  *gm_http.Response
		gmresp, err = c.gmhttpClient.Do(gmreq)
		resp =  (*http.Response)(unsafe.Pointer(gmresp))
...
func (c *Client) StreamResponse(req *http.Request, stream string, cb func(*json.Decoder) error) (err error) {
	...
	//resp, err := c.httpClient.Do(req)
	isGM := gm_x509.GetX509() == gm_x509.GetX509SM2()
	var resp *http.Response
	if  isGM && (c.gmhttpClient!= nil){
		gmreq :=  (*gm_http.Request)(unsafe.Pointer(req))
		var gmresp  *gm_http.Response
