...
import (
	...
	factory "github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric/sdkpatch/cryptosuitebridge"
...
func (msp *bccspmsp) getCertFromPem(idBytes []byte) (*x509.Certificate, error) {
	...
	cert, err := x.GetX509().ParseCertificate(pemCert.Bytes)
	if err != nil{
...
func (msp *bccspmsp) deserializeIdentityInternal(serializedIdentity []byte) (Identity, error) {
	...
	cert, err := x.GetX509().ParseCertificate(bl.Bytes)
	if err != nil {
...
func (msp *bccspmsp) getUniqueValidationChain(cert *x509.Certificate, opts x509.VerifyOptions) ([]*x509.Certificate, error) {
	...
	//validationChains, err := gmX509.GetX509SM2().Verify(cert,opts)
	//if err != nil {
	//	validationChains, err = gmX509.GetX509Std().Verify(cert,opts)
	//	if err != nil{
	...
	validationChains, err :=x.GetX509().Verify(cert, opts)
	if err != nil {
		return nil, errors.WithMessage(err, "the supplied identity is not valid")
	}
...
func (msp *bccspmsp) IsWellFormed(identity *m.SerializedIdentity) error {
	...
	_, err := x.GetX509().ParseCertificate(bl.Bytes)
	return err
}
