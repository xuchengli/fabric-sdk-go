...
func New(config fab.EndpointConfig, opts ...Option) (*Orderer, error) {
	...
	if endpoint.AttemptSecured(orderer.url, orderer.allowInsecure) {
		...
		grpcOpts = append(grpcOpts, grpc.WithTransportCredentials(fabcomm.NewTLS(tlsConfig,logger)))
		//grpcOpts = append(grpcOpts, grpc.WithTransportCredentials(&fabcomm.DynamicClientCredentials{TLSConfig: tlsConfig}))
	} else {
