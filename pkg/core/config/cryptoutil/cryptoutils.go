...
func GetPublicKeyFromCert(cert []byte, cs core.CryptoSuite) (core.Key, error) {
	...
	x509Cert, err := gmX509.GetX509().ParseCertificate(dcert.Bytes)
	if err != nil {
...
func X509KeyPair(certPEMBlock []byte, pk core.Key, cs core.CryptoSuite) (tls.Certificate, error) {
	...
	x509Cert, err := gmX509.GetX509().ParseCertificate(cert.Certificate[0])
	if err != nil {
		logger.Println("Unable to parse cert of gmParse from decoded bytes: %s", err)
		return fail(err)
	}
	...
	case *primitive.Sm2PublicKey:
		cert.PrivateKey = &PrivateKey{cs, pk, &primitive.Sm2PublicKey{}}
	default:
