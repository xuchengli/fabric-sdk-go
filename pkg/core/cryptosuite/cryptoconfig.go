...
// SecurityHashAlgorithm returns cryptoSuite config hash algorithm
func (c *Config) SecurityHashAlgorithm() string {
...
// SecurityAlgorithm returns cryptoSuite config algorithm
func (c *Config) SecurityAlgorithm() string{
	val, ok := c.backend.Lookup("client.BCCSP.security.algorithm")
	if !ok {
		return defAlgorithm
	}
	return cast.ToString(val)
