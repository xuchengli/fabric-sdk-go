...
import (
	...
	"github.com/xuchengli/fabric-sdk-go/internal/github.com/hyperledger/fabric/core/operations"
	...
	sdkApi "github.com/xuchengli/fabric-sdk-go/pkg/fabsdk/api"
	"github.com/xuchengli/fabric-sdk-go/pkg/fabsdk/metrics"
	metricsCfg "github.com/xuchengli/fabric-sdk-go/pkg/fabsdk/metrics/cfg"
	mspImpl "github.com/xuchengli/fabric-sdk-go/pkg/msp"
...
// initializeAlgorithm initializes algorithm config
func (sdk *FabricSDK) initializeAlgorithm(cryptoSuiteConfig core.CryptoSuiteConfig)  error{
	err := config.InitAlgorithm(cryptoSuiteConfig.SecurityAlgorithm())
	if err != nil{
		logger.Error("initializeAlgorithm failed")
		return err
	}
	logger.Info(" initialize algorithm config successfully")
	return nil
}

// initializeX509 Initializes x509
func (sdk * FabricSDK) initializeX509(cryptoSuiteConfig core.CryptoSuiteConfig) error {
	gm_x509.InitX509(cryptoSuiteConfig.SecurityAlgorithm())
	err := gmX509.InitX509(cryptoSuiteConfig.SecurityAlgorithm())
	if err != nil {
		logger.Error("x509 initialized failed")
		return err
	}
	logger.Info("x509 initialized successfully")
...
func (sdk *FabricSDK) loadConfigs(configProvider core.ConfigProvider) (*configs, error) {
	...
	// Initialize crypto algorithm once crypto Suite config is available
	err = sdk.initializeAlgorithm(c.cryptoSuiteConfig)
	if err != nil{
		return nil, errors.WithMessage(err, "unable to initialize algorithm config using crypto suite config")
	}

	// Initialize x509 once crypto Suite config is available
	err = sdk.initializeX509(c.cryptoSuiteConfig)
	if err != nil{
		return nil, errors.WithMessage(err, "unable to initialize x509 using crypto suite config")
	}
